...
Generated by the `improve` skill on 2026-07-29 against commit `fabeb49`. Execute in the order below unless dependencies say otherwise. Every executor must read the whole selected plan, run its drift check, honor STOP conditions, run every verification gate, and update the status row.
